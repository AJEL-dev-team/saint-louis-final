      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5">
        {massTimes.map((mass, index) => (
          <div key={index} className="relative h-full">
            <span
              className={`absolute top-0 left-0 w-full h-full mt-1 ml-1 ${mass.color} rounded-lg`}
            ></span>
            <div
              className={`relative h-full p-5 bg-white border ${mass.borderColor} rounded-lg`}
            >
              ...
              <p
                className={`mt-3 mb-1 text-xs font-medium ${mass.color} uppercase`}
              >
                ------------
              </p>
              <p className="mb-2 text-gray-600">
                Mass Times: {mass.times.join(", ")}
              </p>
            </div>
          </div>
        ))}
